...
parameters{
  ...
  real<lower=0> phi[4]; // dispersion parameters
}
transformed parameters {
  ...
  y = integrate_ode_bdf(
    ...
    for(i in 1:D) {
      output_H[i]   = y[i,5]*pop_t;
      output_ICU[i] = y[i,6]*pop_t;
      output_cumC[i]= (y[i,9]+1.0E-9)*pop_t;
      output_cumX[i]= y[i,8]*pop_t;
      // fitted outputs
      output_k_hospit[i] = i==1 ? output_H[i] : 1.0E-9*pop_t + output_H[i] - 0.0; 
      output_k_icu[i] = i==1 ? output_ICU[i] : 1.0E-9*pop_t + output_ICU[i] - 0.0; 
...
model {
  ...
  // likelihood
  for(i in 1:D) {
    target += neg_binomial_2_lpmf( k_hospit[i] | output_k_hospit[i], phi[1]);
    target += neg_binomial_2_lpmf( k_icu[i] | output_k_icu[i], phi[2]);
    target += neg_binomial_2_lpmf( k_daily_cases[i] | output_k_daily_cases[i], phi[3]);
    target += neg_binomial_2_lpmf( k_daily_deaths[i] | output_k_daily_deaths[i], phi[4]);
  }
}
...
generated quantities{
  ...
  y_pred = integrate_ode_bdf(
    ...
    // for(i in 1:D) {
    //   fitted_k_hospit[i] = neg_binomial_2_rng( output_k_hospit[i], phi[1]);
    //   fitted_k_icu[i]    = neg_binomial_2_rng( output_k_icu[i], phi[2]);
    //   fitted_k_daily_cases[i]  = neg_binomial_2_rng( output_k_daily_cases[i], phi[3]);
    //   fitted_k_daily_deaths[i] = neg_binomial_2_rng( output_k_daily_deaths[i], phi[4]);
    // }
    // for(i in 1:E) {
    //   predicted_k_hospit[i] = neg_binomial_2_rng( comp_H[D+i], phi[1]);
    //   predicted_k_icu[i]    = neg_binomial_2_rng( comp_ICU[D+i], phi[2]);
    //   predicted_k_daily_cases[i]   = neg_binomial_2_rng( comp_diffC[D+i], phi[3]);
    //   predicted_k_daily_deaths[i] = neg_binomial_2_rng( comp_diffX[D+i], phi[4]);
    // }
    for(i in 1:S) {
